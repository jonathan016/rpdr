...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', default='./rpdr-config-results/results/yolo/v3/1_448.log', help='Where to save logs')
    parser.add_argument('-w', default='./rpdr-config-results/results/yolo/v3/1_448.pth.tar',
                        help='Epoch checkpoint output path')
    parser.add_argument('-b', default='./rpdr-config-results/results/yolo/v3/1_448_best_model.pth.tar',
                        help='Weight output path')
    parser.add_argument('-bb', default='./rpdr-config-results/results/yolo/v3/5_256_best_model.pth.tar',
                        help='Backbone weight to finetune')
    parser.add_argument('-tep', default=10, type=int, help='Total epoch of training')
    ...
    from models.proposed import RPYOLOv3
    ...
    best = args.b
    backbone = args.bb
    end_epoch = int(args.tep) + 1
    force_cuda = args.nc

    # Construct model, optimizer, and criterion
    darknet53: RPYOLOv3 = RPYOLOv3()
    darknet53.recognizing()

    if force_cuda and cuda.is_available():
        darknet53 = darknet53.cuda()
    # Load backbone weight to be finetuned
    darknet53.load_state_dict(torch.load(backbone)['state_dict'])
    adam = Adam(darknet53.parameters(), lr=.0001, weight_decay=.0005)
    ...
    for epoch in range(start_epoch, end_epoch):
        darknet53, adam, crit = train(darknet53, adam, crit, epoch, train_loader, val_loader, save, best)
    end_time = time.time()
    out(f'Finetune recognition training elapsed for {end_time - start_time} seconds')
    ...
    # Test model
    darknet53.load_state_dict(torch.load(best)['state_dict'])
    test_loss, test_acc = eval(darknet53, crit, test_loader)
    out('\n=========')
