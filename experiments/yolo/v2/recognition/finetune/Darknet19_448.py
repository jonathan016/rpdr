...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', default='./rpdr-config-results/results/yolo/v2/1_448.log', help='Where to save logs')
    parser.add_argument('-w', default='./rpdr-config-results/results/yolo/v2/1_448.pth.tar',
                        help='Epoch checkpoint output path')
    parser.add_argument('-b', default='./rpdr-config-results/results/yolo/v2/1_448_best_model.pth.tar',
                        help='Weight output path')
    parser.add_argument('-bb', default='./rpdr-config-results/results/yolo/v2/6_224_best_model.pth.tar',
    ...
    from models.proposed import RPYOLOv2
    ...
    darknet19.load_state_dict(torch.load(backbone)['state_dict'])
    ...
    for epoch in range(start_epoch, end_epoch):
        darknet19, adam, crit = train(darknet19, adam, crit, epoch, train_loader, val_loader, save, best)
    end_time = time.time()
    ...
    # Test model
    darknet19.load_state_dict(torch.load(best)['state_dict'])
    test_loss, test_acc = eval(darknet19, crit, test_loader)
    out('\n=========')
