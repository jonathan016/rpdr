...
from runners import RPYOLOv2Runner
...
def train_initial(yaml_config, resume, no_load_resume, logfile):
    ...
    model, optimizer = RPYOLOv2Runner.pre_init({}, learning_rate=lr, momentum=momentum, decay=weight_decay,
    ...
    runner = RPYOLOv2Runner(model, optimizer, dataset, val_dataset, iterations=(start_iter, end_iter),
...
def train_detecting(yaml_config, resume, no_load_resume, logfile):
    ...
    backbone_weight = yaml_config['backbone_weight']
    use_cuda = yaml_config['cuda'] if yaml_config['cuda'] is not None else torch_cuda_is_available()
    anchor_boxes = list(map(float, yaml_config['anchor_boxes']))
    ...
    model, optimizer = RPYOLOv2Runner.pre_init({'anchor_boxes': anchor_boxes}, learning_rate=lr, momentum=momentum,
    ...
    runner = RPYOLOv2Runner(model, optimizer, dataset, validation_dataset=None, iterations=(start_iter, end_iter),
                            batch_size=batch_size, use_cuda=use_cuda, initial_lr=lr, logfile=logfile)

    if resume is None:
        runner.detecting(reset_time=True, backbone_weight=backbone_weight, anchor_boxes=anchor_boxes,
                         decay_steps_scales=list(zip(decay_lr_iterations, decay_lr_scales)))
    else:
        runner.detecting(backbone_weight=backbone_weight, anchor_boxes=anchor_boxes,
                         decay_steps_scales=list(zip(decay_lr_iterations, decay_lr_scales)))
        if no_load_resume:
            no_load_resume = no_load_resume.split(',')
            runner.load(resume, no_load_resume)
