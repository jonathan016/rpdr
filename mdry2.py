...
from runners import MDRY2Runner
...
def train_initial(yaml_config, resume, no_load_resume, logfile):
    ...
    model, optimizer = MDRY2Runner.pre_init({}, learning_rate=lr, momentum=momentum, decay=weight_decay,
    ...
    runner = MDRY2Runner(model, optimizer, dataset, val_dataset, iterations=(start_iter, end_iter),
...
def train_detecting(yaml_config, resume, no_load_resume, logfile):
    ...
    model, optimizer = MDRY2Runner.pre_init({'anchor_boxes': anchor_boxes_config}, learning_rate=lr,
                                            momentum=momentum, decay=weight_decay, batch_size=batch_size)
    ...
    runner = MDRY2Runner(model, optimizer, dataset, validation_dataset=None, iterations=(start_iter, end_iter),
                         batch_size=batch_size, use_cuda=use_cuda, initial_lr=lr, logfile=logfile)
