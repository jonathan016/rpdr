...
from models import RPYOLOv2

from math import isnan


class RPYOLOv2Runner:
    @staticmethod
    def pre_init(model_kwargs, learning_rate, batch_size, decay, momentum):
        ...
        model: RPYOLOv2 = RPYOLOv2(**model_kwargs)
    ...
    def _handle_backbone_weight_type(backbone_weight):
        if type(backbone_weight) == str:
            return torch_load(backbone_weight)['model']
        elif type(backbone_weight) == OrderedDict:
            return backbone_weight
        else:
            raise ValueError('backbone_weight type is unknown')

    def __init__(self, model: RPYOLOv2, optimizer, dataset, validation_dataset, batch_size=64, iterations: tuple = None,
    ...
    def detecting(self, reset_time=False, backbone_weight=None, anchor_boxes=None, dataset=None, batch_size=None,
                  iterations: tuple = None, initial_lr=None, decay_steps_scales: List[Tuple] = None, seen_images=None,
        ...
        if anchor_boxes:
            self.model.anchor_boxes = anchor_boxes
    ...
    def train_(self, model, optimizer, loader, savefile):
        ...
        for data, target in loader:
            ...
            torch_save({
                ...
                'loss_function': model.loss_function.state_dict(),
    ...
    def load(self, checkpoint, not_to_load=None):
        ...
        if 'loss_function' in loader.keys():
            self.model.loss_function.load_state_dict(loader['loss_function'])
        ...
        if 'seen_images' in loader.keys():
            ...
            if not self.model.is_recognizing():
                self.model.loss_function.set_seen_images(self.seen_images)
